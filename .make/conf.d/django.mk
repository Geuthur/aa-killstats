...
pot: check-python-venv
	...
	sed -i "/\"Project-Id-Version: /c\\\"Project-Id-Version: $(appname_verbose) $$current_app_version\\\"" $(translation_template); \
...
translations: check-python-venv
	...
	@current_app_version=$$(pip show $(appname) | grep 'Version: ' | awk '{print $$NF}'); \
	sed -i "/\"Project-Id-Version: /c\\\"Project-Id-Version: $(appname_verbose) $$current_app_version\\\"" $(translation_template); \
	sed -i "/\"Report-Msgid-Bugs-To: /c\\\"Report-Msgid-Bugs-To: $(git_repository_issues)\\\"" $(translation_template); \
	...
	if [ $$subdircount -gt 1 ]; then \
		for path in $(translation_directory)/*/; do \
			...
			if [ -f "$$path/$(translation_file_relative_path)" ]; then \
				sed -i "/\"Project-Id-Version: /c\\\"Project-Id-Version: $(appname_verbose) $$current_app_version\\\"" $$path/$(translation_file_relative_path); \
				sed -i "/\"Report-Msgid-Bugs-To: /c\\\"Report-Msgid-Bugs-To: $(git_repository_issues)\\\"" $$path/$(translation_file_relative_path); \
			fi; \
		done; \
	fi;
